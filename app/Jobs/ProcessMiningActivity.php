...
class ProcessMiningActivity implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $corporationId = (int)config('eve.tax_corporation_id');
        ...
        Log::info('ProcessMiningActivity: found mining activity entries to process', ['count' => count($activity)]);
        ...
        foreach ($miner_data as $key => $value) {
            ...
            if ($miner) {
                ...
                $miner->save();
                Log::info('ProcessMiningActivity: updated amount owed', [
                    'char_id' => $key,
                    'diff' => number_format($value) . ' ISK',
                    'total' => number_format($miner->amount_owed) . ' ISK',
                ]);
            }
        }
        ...
        foreach ($refinery_data as $key => $value) {
            $refinery = Refinery::where('observer_id', $key)->first(); /* @var Refinery $refinery */
            $refinery->income += $value;
            $refinery->save();
            Log::info('ProcessMiningActivity: updated stored amount generated by refinery', [
                'refinery_id' => $key,
                'total' => number_format($refinery->income) . ' ISK',
            ]);
        }
    }
}
